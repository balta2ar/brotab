...
class BrowserTabs {
  ...
  list(queryInfo, onSuccess) {
    browser.tabs
      .query(queryInfo)
      .then(onSuccess, (error) => console.log(`Error listing tabs: ${error}`));
  }
  ...
  close(tab_ids, onSuccess) {
    browser.tabs
      .remove(tab_ids)
      .then(onSuccess, (error) => console.log(`Error removing tab: ${error}`));
  }

  move(tabId, moveOptions, onSuccess) {
    browser.tabs.move(tabId, moveOptions).then(
      onSuccess,
  ...
  update(tabId, options, onSuccess, onError) {
    browser.tabs.update(tabId, options).then(onSuccess, (error) => {
  ...
  runScript(tab_id, script, payload, onSuccess, onError) {
    browser.tabs.executeScript(tab_id, { code: script }).then(
...
if (typeof chrome !== "undefined") {
  console.log("It's Chrome/Chromium");
  browserTabs = new BrowserTabs("chrome/chromium");
} else {
  browserTabs = new BrowserTabs("firefox");
}

var port: browser.Runtime.Port = undefined;
...
function queryTabsOnSuccess(tabs) {
  tabs.sort(compareWindowIdTabId);
  let lines = tabs.map(
    (tab) => `${tab.windowId}.${tab.id}\t${tab.title}\t${tab.url}`
  );
  console.log(lines);
  port.postMessage(lines);
}
  ...
  try {
    ...
    query = Object.entries(query).reduce((o, [k, v]) => {
      if (booleanKeys.hasOwnProperty(k) && typeof v != "boolean") {
        if (v.toLowerCase() == "true") o[k] = true;
        else if (v.toLowerCase() == "false") o[k] = false;
        else o[k] = v;
...
function moveTabs(move_triplets) {
  ...
  browserTabs.move(tabId, { index: index, windowId: windowId }, (tab) =>
    moveTabs(move_triplets.slice(1))
  );
}
  ...
  for (let url of urls) {
    ...
    promises.push(
      new Promise((resolve, reject) => {
        browserTabs.create({ url: url, windowId: window_id }, (tab) =>
          resolve(`${tab.windowId}.${tab.id}`)
        );
      })
    );
  }
...
function createTab(url) {
  browserTabs.create({ url: url }, (tab) => {
    ...
    promises.push(
      new Promise((resolve, reject) => {
        browserTabs.update(
          ...
          (error) => {
            console.error(
              `Could not update tab: ${error}, update=${JSON.stringify(update)}`
            );
            resolve();
          }
        );
...
function getWordsScript(match_regex, join_with) {
  return GET_WORDS_SCRIPT.replace("#match_regex#", match_regex).replace(
    "#join_with#",
    join_with
  );
}
...
function getTextScript(delimiter_regex, replace_with) {
  return GET_TEXT_SCRIPT.replace("#delimiter_regex#", delimiter_regex).replace(
    "#replace_with#",
    replace_with
  );
}
...
function getHtmlScript(delimiter_regex, replace_with) {
  return GET_HTML_SCRIPT.replace("#delimiter_regex#", delimiter_regex).replace(
    "#replace_with#",
    replace_with
  );
}
...
function getWordsFromTabs(tabs, match_regex, join_with) {
  ...
  Promise.all(promises).then((all_words) => {
...
function getWords(tab_id, match_regex, join_with) {
  if (tab_id == null) {
    ...
    browserTabs.getActive((tabs) =>
      getWordsFromTabs(tabs, match_regex, join_with)
    );
  } else {
    const script = getWordsScript(match_regex, join_with);
    ...
    browserTabs.runScript(
      ...
      (error, _payload) =>
        console.log(
          `getWords: tab_id=${tab_id}, could not run script (${script})`
        )
    );
  }
}

function getTextOrHtmlFromTabs(
  tabs,
  scriptGetter,
  delimiter_regex,
  replace_with,
    ...
    var promise = new Promise((resolve, reject) =>
      browserTabs.runScript(
        ...
        (text, current_tab) => {
          ...
          if (text && text[0]) {
            console.log(
              `Got ${text.length} chars of text from another tab: ${current_tab.id}`
            );
        ...
        (error, current_tab) => {
          console.log(
            `Could not get text from tab: ${error}: ${current_tab.id}`
          );
          resolve({ tab: current_tab, text: "" });
        }
      )
...
function getTextOnListSuccess(tabs, delimiter_regex, replace_with) {
  // Make sure tabs are sorted by their index within a window
  tabs.sort(compareWindowIdTabId);
  getTextOrHtmlFromTabs(
    tabs,
    getTextScript,
    delimiter_regex,
    replace_with,
    getTextOnRunScriptSuccess
  );
}
...
port.onDisconnect.addListener(function () {
  ...
  if (browser.runtime.lastError) {
    console.warn("Reason: " + browser.runtime.lastError.message);
  } else {
